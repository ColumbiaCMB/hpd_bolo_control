...
class data_logging:
    ...
    def open_file(self,suffix=None):
        ...
        temp_name = time_now.strftime("%Y%m%d_%H%M%S")
        if suffix is None:
            file_name = temp_name + ".nc"
        else:
            file_name = temp_name + "_" + suffix + ".nc"

        self.rootgrp = Dataset(file_name, 'w', format='NETCDF4')
        self.top_registers = {}
        self.stream_sources = {}
    ...
    def addStream(self,name,st_buffer,speed):
        ...
        self.stream_sources[name] = st_buffer
        self.rootgrp.createVariable(name,'f4',(speed),
                                    zlib=True,chunksizes=[self.chunksize])

    def addGroup(self,name,registers):
    ...
    def add_IV_data(self,I,V,mjd,suffix=None):
        ...
        group_name = "IV_%i" % mjd[0] #cast to float
        group = self.rootgrp.createGroup(group_name)
    ...
    def log_data(self):
        ...
        for stream_name in self.stream_sources:
            stream_size =  len(self.stream_sources[stream_name])
            if stream_size == 0:
                break
            ...
            data = list(self.stream_sources[stream_name])
            self.stream_sources[stream_name].clear()
